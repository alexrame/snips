...
def neuraln(trX,trY,params,size_output=4,nb_step=50,nbLay=5,nbNodes=200,p_drop_input=0.5,p_drop_hidden=0.5,lambda1=0.00000001,lambda2=0.00005,teX=[],teY=[],lr=0.001):
    ...
    while i<nb_step:
        ...
        if i%(nb_step/20)==0:
            ...
            if not len(teX)==0:
                scoreTe=np.mean(teY == predict(teX))
                print "scoreTe", scoreTe

    return params

class NeuralNet():  
    def __init__(self,size_input,size_output=4,nb_step=200,nbLay=5,nbNodes=150,p_drop_input=0.5,p_drop_hidden=0.5):
    ...
    def fit(self, trX,trY,lambda1=0.00000001,lambda2=0.00005,new=0,teX=[],teY=[],lr=0.001):
        if not new:
